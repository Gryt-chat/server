...
export async function processUploadedImage(
  ...
  const rawBuffer = await getObjectAsBuffer({ bucket, key: rawKey });
  ...
  if (!shouldKeepOriginal && hasLimit && rawBytes > maxBytes) {
    ...
    if (avifBuf.length <= maxBytes) {
      ...
      await putObject({ bucket, key: newKey, body: avifBuf, contentType: "image/avif" });

      if (newKey !== rawKey) {
        await deleteObject({ bucket, key: rawKey }).catch(() => {});
      }
    }
  }
